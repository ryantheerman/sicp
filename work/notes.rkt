...
; CS61A LECTURE 01 ##
; this is new.
; got through sicp up to section 1.2.2, but sometimes getting a bit lost
; so i'm going to follow along with the 2011 berkeley cs61a lectures by brian harvey, using sicp as the textbook.
; will cover the already covered material in line with the lectures, then pick up where i left off at section 1.2.3
; note: had been using the racket sicp package
;       now using the planet-dyoo-simply-scheme1 package
...
; notice with sentence, this is something scheme is printing out
; if i entered (now here), it would not print the sentence "now here"
; if would call the function now with the parameter here
; if i want to print a sentence without calling sentence...
'(magical mystery tour)

; applying first, butfirst, etc to 'sentences will output the first word, or all but the first word, or the last word, etc
(first '(got to get you in my life))
...
; can give names to things in scheme:
...
; in scheme, the rule of evaluation is that the argument expressions are evaluated first (above (+ 2 3) must be evaluated before square, for example)
; but there are a few exceptions.
; one of them being define itself
; above, when i write (define pi), pi has no value, so it cannot be evaluated
; same for (define (square x)).
; if i haven't defined square, (square x) means nothing
; the define function is a special form.
; 'define' itself is the keyword of the special form
; what's special about it is that it does not first evaluate the arguments
...
; above we are defining a variable.
; if hello were instead in parentheses, we'd be defining a function, and nothing would be evaluated until we call the function.
...
(define (square x) ; x is the formal parameter of the function
  (* x x)) ; (* x x) is the body of the function

(square (+ 2 3)) ; (+ 2 3) is the actual argument expression
; 5 is the actual argument value

; 3 things that can loosely be called the argument of the function
; the formal parameter, the actual argument expression, or 5 which is the actual argument value 
; sometimes we'll just call it the argument, or the input
...
; all the words and sentences stuff is berkeley flavor
; ordinary scheme doesn't include this, but it's helpful for demonstration and experimentation.
; berkeley imported the idea of words and sentences from logo

; pig latin function
(define (pigl wd)
  ; if pig latin is done?
  (if (pl-done? wd)
      ; return the input word with the word 'ay at the end
      (word wd 'ay)
      ; otherwise, call pigl on a new input word, which is all butfirst of the current input word with the first of the current input word at the end
      (pigl (word (bf wd) (first wd)))))

; pig latin is done if the first letter of the input word is a vowel
(define (pl-done? wd)
  (vowel? (first wd)))

; a letter is a vowel if it is a member of the sentence '(a e i o u)
(define (vowel? letter)
...
; it has to be;
; if we evaluated the recursive call (pigl) before we evaluated if, even when we got to the base case we couldn't stop evaluating. we'd evaluate forever because we'd keep needing to evaluate the recursive call.
; but a special form is special because it's evaluation rules are different from normal functions.
; in pigl, if is evaluated first.
; if true, then the base case is evaluated
...
; let's make a procedure to compute factorials...
; one way to compute a factorial is to use the fact that n! = n * (n - 1)!
; we can compute (n - 1)! and multiply the result by n
; we'll add the stipulation that 1! = 1...
...
; what are we looking at here? understand it...
; we're defining a procedure called factorial which takes one input called n
; the body of the procedure states that if n = 1, return 1
; else return product of n multiplied by the output of the factorial function called on n - 1

; so in a simple case, say we run factorial 3
; 3 != 1, so we multiply 3 by factorial 2.
; we need to hold onto that expression because we can't return an output yet...
; first we need to run factorial 2
; 2 != 1 so we multiply 2 by factorial 1...
; which we need to compute...
; 1 = 1, so factorial 1 returns 1
; which we multiply by that layer's n, which is 2 to get 2...
; which we multiply by the original n, 3, to get 6
...
; the above shows the substitution model of computing 6!
; this is called a _linear recursive process_

; but we can look at it from another perspective.
; let's describe a rule for computing n! by specifying that we first multiply 1 by 2, then multiply the result by 3, then by 4, and so on until we reach n.
; we maintain a running product together with a counter that counts up from 1 to n
; the counter and the product change simultaneously at each step according the this rule:
...
; break it down
; we define a function called factorial which takes input n as the function fact-iter taking inputs 1, 1, and n
; we define function fact-iter which takes 3 inputs: product, counter, and max-count
; fact-iter checks first if counter is greater than max-count
; if true, it returns the product
; if false, it returns the output of the call to fact-iter with inputs counter times product for product, counter plus 1 for counter, and max-count as max-count
; using this function, let's compute factorial 3
; factorial 3 calls (fact-iter 1 1 3)
; 1 is the starting product, 1 is the starting counter, and 3 is the max-count
; 1 is not greater than max-count, so we call fact-iter with inputs 1, 2, and 3
; 2 is not greater than max-count, so we call fact-iter with inputs 2, 3, and 3
; 3 is not greater than max-count, so we call fact-iter with inputs 6, 4, and 3
; 4 is greater than 3, so we return the current value of product, which is 6
...
; (fact-iter 720 7 6)
; 720
; this is the _linear iterative process_ for computing 6!

; the linear recursive and linear iterative processes look similar
; they both compute the same mathematical function on the same domain
; each requires a number of steps proportional to n to compute n!
; they even carry out the same sequence of multiplications
; but the "shapes" of the two processes show they evolve very differently

; in the linear recursive process, the interpreter expands then contracts
; the process builds up a chain of _deferred operations_ (the chain of multiplications)
; this is because we need the output of the recursive calls in order to calculate the outer calls
; so we expand until there are no more recursive calls to make...
; then contract as the deferred operations occur
; KEY POINT:
; the interpreter needs to keep track of all the operations that need to be performed.
; the length of the chain of calculations to compute n! grows linearly with n. it is proportional to n.

; on the other hand, in the linear iterative process we don't maintain a chain calculations as deferred operations
; we don't expand and contract
; all we need to keep track of at any point in the process are the current values of product, counter, and max-count
; we need to keep track of those variables' state
...
; like the linear recursive process, the number of steps to compute n! grows linearly with n, but there is no chain of deferred operations to keep track of.

; if the linear iterative process is interrupted, all we need to do to pick up where we left off is to supply the state of the variables at the time of termination.
; with the linear recursive process however, we could not pick up part way through. we'd have to start from the beginning. the interpreter is maintaining some "hidden" information (where in the process we are), that is lost if the process is interrupted. the longer the chain of deferred operations, the more information must be maintained by the interpreter.

; NOTE: when contrasting iteration and recursion, do not confuse a recursive _process_ with a recursive _procedure_.
; a recursive procedure means that the procedure definition refers, either directly or indirectly, to the procedure itself.
; a recursive process refers to how the process evolves, not the syntax of how a procedure is written.
...
; quick note. i'm pivoting to the following along with the 2011 cs61a lectures from berkeley using sicp as the textbook, rather than working through sicp solo. i need some structure (in the structure)
; going to retread the ground i've already covered and augment the notes as i go. will pick up here when i get back.
; note on the scheme variant: the lecturer uses a different variant of scheme than i've been using. i found it available as a racket package called planet-dyoo-simply-scheme1. so i installed that racket package and have updated my sicp script to launch that repl instead of sicp.
; some things are missing, like inc and dec, but those are easy to implement inline if needed. the simply-scheme package gets me the simple word primitives demonstrated in the first lecture. if i'm using the lectures as my guide i'd rather be more aligned with them than the sicp book.
